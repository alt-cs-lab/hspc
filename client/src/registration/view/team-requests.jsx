...
class SchoolRequests extends Component {
    ...
    */
        ...
        return [
            ...
            },
            {
                ...
                cell: row => {
                    ...
                    return(
                        <div>
                            Beginner: {teamcaps.beginnerteamcount}/{this.state.beginnerSchoolCapacity} Advanced: {teamcaps.advancedteamcount}/{this.state.advancedSchoolCapacity} Total: {teamcaps.totalteamcount}/{this.state.schoolCapacity}
                        </div>
                    )
                }
            },
            ...
            },
            {
                ...
                cell: row => {
                return(
                    <Button onClick={() => {this.handleCompleteRequest(true, row.teamid);}}>
                        Register
                    </Button>
                    );
                },
            ...
            },
            {
                ...
                cell: row => {
                    return(
                    <Button variant="secondary" onClick={() => this.handleCompleteRequest(false, row.teamid)} >
                        Deny
                    </Button>
                    );
                },
    ...
    getSchoolTeamCapacities(schoolid){
        if(Object.keys(this.state.schoolTeamCountList).length > 0 && this.state.schoolTeamCountList[schoolid] !== null) return this.state.schoolTeamCountList[schoolid];
        else return {
            beginnerteamcount: 0,
            advancedteamcount: 0,
            totalteamcount: 0,
    ...
    */
    UpdateCompetition = (eventid, teamsPerEvent, beginnerTeamsPerEvent, advancedTeamsPerEvent, teamsPerSchool, beginnerTeamsPerSchool, advancedTeamsPerSchool) => {
        var requestTable = [];
        ...
        .then((response) => {
            ...
            } else console.log("An error has occurred retrieving team competition capacities, Please try again.");
            ...
            .then((response) => {
                ...
                } else console.log("An error has occurred retrieving team school capacities, Please try again.");
    
                RequestService.getWaitlistedTeamsForEvent( eventid )
                .then((response) => {
                    if (response.ok) {
                        requestTable = response.data;
                    } else console.log("An error has occurred retrieving waitlisted teams, Please try again.");
            ...
            <div>
                ...
                <div class="add-margin">
                    <Select
                        ...
                        onChange={target => this.UpdateCompetition(target.value, target.teamsPerEvent, target.beginnerTeamsPerEvent, target.advancedTeamsPerEvent, target.teamsPerSchool, target.beginnerTeamsPerSchool, target.advancedTeamsPerSchool)} />
                </div>
                <br/>
                ...
                <div>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-evenly" }}>
                        <div className="mb-3" style={{ margin: "4px" }}>
                            ...
                            <h6>{this.state.totalTeamCount}/{this.state.teamCapacity}</h6>
                        </div>
                        <hr width="1" size="100" />
                        <div className="mb-3" style={{ margin: "4px" }}>
                            <h5>Beginner Event Capacity</h5>
                            <h6>{this.state.totalBeginnerCount}/{this.state.beginnerCapacity}</h6>
                        </div>
                        <hr width="1" size="100" />
                        <div className="mb-3" style={{ margin: "4px" }}>
                            <h5>Advanced Event Capacity</h5>
                            <h6>{this.state.totalAdvancedCount}/{this.state.advancedCapacity}</h6>
                        </div>
                    </div>
    ...
    useEffect(() => {
        ...
        .then((response) => {
            if (response.ok) {
                ...
                setAdvisorName( response.data.advisorfirstname + ' ' + response.data.advisorlastname )
