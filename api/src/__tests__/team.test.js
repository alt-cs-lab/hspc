...
describe("Team Testing Suite", () => {
    ...
    describe("post", () => {
        ...
        it("should successfully create a new team when given valid data by an advisor", async () => {
            ...
            const getResponse = await request.get("/api/team").set("Authorization", getAdminToken());
            ...
            expect(getResponse.body[6].questionLevelId).toBe(1);
        ...
        it("should fail to create a new team for a school at max teams", async () => {
            ...
            // should fail now since max cfor competition 1 is 3 and 1 was already in the db 
            const newTeamResponse3 = await request.post("/api/team").set("Authorization", getAdminToken()).send(team);
            // expect teamId to be 8
            expect(newTeamResponse3.statusCode).toBe(400);
        });
    });
    ...
    describe("delete", () => {
        ...
        it("should successfully delete an existing team when called by an advisor", async () => {
            const deleteTeamResponse = await request.delete("/api/team?teamId=6").set("Authorization", getAdminToken());
        ...
        it("should delete just the student from the team if studentId is passed in", async () => {
            ...
            const deleteStudentResponse = await request.delete("/api/team?teamId=1&studentId=3").set("Authorization", getAdminToken());
